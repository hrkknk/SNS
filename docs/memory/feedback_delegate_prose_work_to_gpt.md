...
description: 文章の校正・アドバイスは自分でやらず、Cursor の GPT（Task / gpt-5.6-luna-high）に委譲する
metadata: 
  node_type: memory
  type: feedback
...
記事など本人の文章に関する**校正・言い回しの助言・講評は、親 Agent（Cursor Auto）が自分でやらない**。Cursor の Task で gpt-5.6-luna-high に投げ、結果をそのまま本人に渡す。親の担当は、原稿の取得（Notion API / MCP）、GPT への受け渡し、本人が採用すると決めた分だけの反映、検証。
...
当初の呼び出しは OpenAI の Codex CLI（`codex exec` / gpt-5.6-terra）だった。2026-08-28、本人が GPT を直接使うのをやめたため、Cursor の Task に切り替えた。Task に terra は無く、sol は high / xhigh しか無い。2026-08-29、sol は重すぎるとの判断で **`gpt-5.6-luna-high`** に落とした。委譲する理由（親が本文を書かない）は変わらない。
