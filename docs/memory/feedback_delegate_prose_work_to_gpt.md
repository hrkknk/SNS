...
当初の呼び出しは OpenAI の Codex CLI（`codex exec` / gpt-5.6-terra）だった。2026-08-28、本人が GPT を直接使うのをやめたため Cursor の Task に切り替えた。2026-08-29、コストと自然言語の兼ね合いで委譲先を **`gpt-5.6-luna-high`** にした。委譲する理由（親が本文を書かない）は変わらない。
